...
! Open HBOOK/NTUPLE file here
	if(hut_ntuple) then
	   ...
	   elseif(ispec.eq.3) then
	      call hrsl_hbook_init(hbook_filename,spec_ntuple)
	   else
...
c	  call srand(itime)
	...
	do Itrial = 1,n_trials
	   ...
	   elseif(ispec.eq.3) then
	      armSTOP_successes=lSTOP_successes
	   endif
...
c          rnum=9*grnd() 
c          do it=1,9
c          if (rnum .ge. it-1 .and. rnum .lt. it) z =-16+(it-1)*4
c          enddo

c          rnum=grnd()
c          if (rnum .lt. 0.333) z =z + 20
c          if (rnum .gt. 0.666) z =z - 20
c          if (rnum .gt. 0.5) z =z - 5
c          if (rnum .le. 0.5) z =z + 5
...
c
	  ...
	  if(ispec.eq.2 .or. ispec.eq.3) then ! SHMS
...
C Calculate multiple scattering length of target
	  ...
	  elseif(ispec.eq.2 .or. ispec.eq.3) then ! spectrometer on left
	     cos_ev = (cos_ts-dydz_s*sin_ts)/sqrt(1+dydz_s**2+dxdz_s**2)
	  endif
	  th_ev = acos(cos_ev)
	  sin_ev = sin(th_ev)
...
c            if (ok_spec) spec(58) =1.
     ...
     >          pathlen)
	  elseif(ispec.eq.3) then
	     call mc_hrsl(p_spec, th_spec, dpp_s, x_s, y_s, z_s, 
     >          dxdz_s, dydz_s,
     >          x_fp, dx_fp, y_fp, dy_fp, m2,
     >          ms_flag, wcs_flag, decay_flag, resmult, fry, ok_spec, 
     >          pathlen)
...
C for spectrometer ntuples
	 ...
	 if(ispec.eq.3) then
	    if (store_all.OR.(hut_ntuple.AND.ok_spec)) then
	       ...
	       hrsl_hut(3) = dx_fp
	       hrsl_hut(4) = dy_fp
	       hrsl_hut(5) = ytar_init
	       hrsl_hut(6) = dth_init/1000.
	       hrsl_hut(7) = dph_init/1000.
	       hrsl_hut(8) = 0.
	       hrsl_hut(9) = 0.
c	       hrsl_hut(8) = ytar_recon
c	       hrsl_hut(9) = dpp_init
c	       hrsl_hut(10)= dpp_recon
c	       hrsl_hut(11)= dth_recon/1000.
c	       hrsl_hut(12)= dph_recon/1000.
c	       hrsl_hut(13) = fry
c	       hrsl_hut(14)= ztar_init
	       call hfn(1500,hrsl_hut)
	    endif
	 endif
...
c        A=1.
	 Mp =938.27
           write(*,*) " mc ",qsq/1.d6, sqrt(wsq/1.d6)
	 if ( w .gt. 1075.) then
