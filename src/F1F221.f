...
CCC///    Integrate over y    ///CCC
      ...
      f1s = dsimps(real(vfact1,16),real(ymin,16),real(ymax,16),nbins)/2./x  !!! F1d
      f2s = dsimps(real(vfact2,16),real(ymin,16),real(ymax,16),nbins)       !!! F2d
...
C ***********************************************************************
      ...
      real*8 f2tildav(10001),f1os,f2os,dgauss,pvinc,wfnorm(10)
      real*8 U,W,VS,VT
      real*16 dsimps
...
c      write(6,*) w2,q2,wfn
      ...
      pvmin = mp*prod*(1.d0-rt)*sign(real(1, kind(a)),(a-b))
...
c        write(6,*) "offshellqe:  ", x,pv,p2,xth,pvmin,pvmax
      ...
      ft = dsimps(real(fttildav,16),real(pvmin,16),real(pvmax,16),nbins)
      f2os = dsimps(real(f2tildav,16),real(pvmin,16),real(pvmax,16),nbins)
...
*
* $Id: dsimps64.F,v 1.1.1.1 1996/04/01 15:02:13 mclareni Exp $
*
* $Log: dsimps64.F,v $
...
*
* $Id: dsimpscod.inc,v 1.1.1.1 1996/04/01 15:02:13 mclareni Exp $
*
* $Log: dsimpscod.inc,v $
* Revision 1.1.1.1  1996/04/01 15:02:13  mclareni
* Mathlib gen
...
c	u2 = DQDVAL (q,nq,qgrid,wqgrid,.FALSE.)
 999	CALL Pinterp (real(qgrid,16),real(uqgrid,16),nq,real(q,16),real(u0,16),real(dum,16),2)
	CALL Pinterp (real(qgrid,16),real(wqgrid,16),nq,real(q,16),real(u2,16),real(dum,16),2)
...
c        print *, '... AV18 data read ...'
 ...
 123	IF (p.LE.Parr(1)) rho = 4*pi * RHOarr(1)
	IF (p.GT.Parr(1) .AND. p.LE.Parr(np)) THEN
	  CALL Pinterp (real(Parr,16),real(RHOarr,16),np,real(p,16),real(rho_int,16),real(dum,16),2)
	  rho = 4*pi * rho_int
...
c	vs = DQDVAL (q,nq,qgrid,vsgrid,.FALSE.)
 999	CALL Pinterp (real(qgrid,16),real(ugrid,16),nq,real(q,16),real(u,16),real(dum,16),1)
	CALL Pinterp (real(qgrid,16),real(wgrid,16),nq,real(q,16),real(w,16),real(dum,16),1)
	CALL Pinterp (real(qgrid,16),real(vtgrid,16),nq,real(q,16),real(vt,16),real(dum,16),1)
	CALL Pinterp (real(qgrid,16),real(vsgrid,16),nq,real(q,16),real(vs,16),real(dum,16),1)
